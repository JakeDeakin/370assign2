 ...
 * series of reports used by the logger. testing
 * 
 * @author hogan
 * 
 */
public class CarPark {
	...
	public void archiveDepartingVehicles(int time, boolean force)
		...
		if (force) { 
			...
			for (int i = 0; i < spaces.size(); i++) {
				Vehicle v = spaces.get(0);
				if (!v.isParked()) {
					throw new VehicleException("Vehicle is not parked.");
				}
				unparkVehicle(v, time);
				past.add(v);
				i--; // because an element was just removed, this will make sure
						// the program checks the index again.
			}
		} else {
			for (int i = 0; i < spaces.size(); i++) {
				...
				if (v.getDepartureTime() <= time) {
					unparkVehicle(v, time);
					past.add(v);
					i--; // because an element was just removed, this will make
							// sure the program checks the index again.
				}
	...
	public void enterQueue(Vehicle v) throws SimulationException,
		...
		if (v.isParked() || v.isQueued()) {
			throw new VehicleException(
					"Vehicle is already parked or in a queue.");
		}
		v.enterQueuedState();
		queue.add(v);
	}
	...
	public void exitQueue(Vehicle v, int exitTime) throws SimulationException,
		...
		if (v.getArrivalTime() > exitTime) {
			throw new VehicleException(
					"The exit time cannot be less than the arrival time.");
		}
	...
	public void parkVehicle(Vehicle v, int time, int intendedDuration)
		...
		if (v.isParked() || v.isQueued()) {
			throw new VehicleException(
					"The vehicle cannot be queued or parked.");
		}
		if (intendedDuration < Constants.MINIMUM_STAY) {
			throw new VehicleException(
					"The intended duration cannot be less than the minimum stay time.");
		}
	...
	public void processQueue(int time, Simulator sim) throws VehicleException,
		...
		if (queue.size() != 0) {
			...
			if (v.getArrivalTime() > time) {
				throw new VehicleException(
						"The arrival time cannot be greater than the current time.");
			}
	...
	public boolean spacesAvailable(Vehicle v) {
		...
		if (v instanceof Car) {
			if (!((Car) v).isSmall() && carSpacesAvailable > 0) {
				return true;
			} else if (((Car) v).isSmall() && smallCarSpacesAvailable > 0
					|| carSpacesAvailable > 0) {
				return true;
			} else {
				return false;
			}
	 ...
	 *             constraints
	 * @throws SimulationException
	 *             if vehicle is not in car park //TODO
	 * 
	 * @author Jake n8509956 and Jamie n8853312
	 */
	public void unparkVehicle(Vehicle v, int departureTime)
		...
		if (departureTime < v.getParkingTime()) {
			throw new VehicleException(
					"departureTime cannot be less than the parkingTime");
		}
